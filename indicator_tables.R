...
#Childhood
diarrhea_table <- kr_fish_gps %>% tab(diarrhea, h11)
dpt_table <- kr_fish_gps %>% tab(dpt, h7)
polio_table <- kr_fish_gps %>% tab(polio, h6)
meas_table <- kr_fish_gps %>% tab(meas, h9)
immun_table <- kr_fish_gps %>% tab(immun, dpt, polio, meas)
not_immun_table <- kr_fish_gps %>% tab(not_immun, dpt, polio, meas)
ari_table <- kr_fish_gps %>% tab(ari, h31b, h31c)
fever_table <- kr_fish_gps %>% tab(fever, h22)
bfeeding_table <- kr_fish_gps %>% tab(bfeeding, m4)
